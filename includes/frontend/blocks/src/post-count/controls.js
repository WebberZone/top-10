...
const Controls = ( { attributes, setAttributes } ) => {
	...
	const [ isFromDatePickerVisible, setIsFromDatePickerVisible ] =
		useState( false );
	const [ isToDatePickerVisible, setIsToDatePickerVisible ] =
		useState( false );
			...
			<InspectorControls>
				...
				>
					<SelectControl
						...
						onChange={ ( value ) =>
							setAttributes( { counter: value } )
						}
					/>
					{ counter === 'daily' && (
						<>
							<div className="tptn-date-picker-container">
								<span className="tptn-date-picker-label">
									{ __( 'From', 'top-10' ) }
								</span>
								<Button
									onClick={ () =>
				...
				>
					...
					{ advancedMode && (
						<TextareaControl
							...
							// translators: %totalcount% and %dailycount% are literal block placeholders.
							help={ __(
