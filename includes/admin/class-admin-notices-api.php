...
class Admin_Notices_API {
	...
	public function __construct() {
		...
		Hook_Registry::add_action( 'wp_ajax_tptn_dismiss_notice', array( $this, 'handle_notice_dismissal' ) );
	}
	...
	private function print_scripts() {
		...
		jQuery(document).ready(function($) {
			$('.notice[data-notice-id]').on('click', '.notice-dismiss', function() {
				...
				$.post(ajaxurl, {
					action: 'tptn_dismiss_notice',
					notice_id: noticeId,
					dismiss_time: dismissTime,
					nonce: '<?php echo esc_js( wp_create_nonce( 'tptn_dismiss_notice' ) ); ?>'
				});
			});
		});
		</script>
	...
	public function handle_notice_dismissal() {
		check_ajax_referer( 'tptn_dismiss_notice', 'nonce' );
		...
		if ( $dismiss_time ) {
			set_transient( "tptn_notice_dismissed_{$notice_id}", true, $dismiss_time );
		} else {
			update_user_meta( get_current_user_id(), "tptn_notice_dismissed_{$notice_id}", true );
		}
	...
	private function is_notice_dismissed( $notice_id ) {
		...
		if ( $notice['dismiss_time'] ) {
			return (bool) get_transient( "tptn_notice_dismissed_{$notice_id}" );
		}

		return (bool) get_user_meta( get_current_user_id(), "tptn_notice_dismissed_{$notice_id}", true );
	}
}
